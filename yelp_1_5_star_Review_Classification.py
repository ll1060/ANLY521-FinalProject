...
def main(yelp_reivew):
    #read in the dataset
    yelp_reviews = pd.read_csv(args.yelp_review)

    # filter the data to contain only 1 star and 5 star reviews
    df_1_5_star = yelp_reviews[(yelp_reviews['stars'] == 1.0) | (yelp_reviews['stars'] == 5.0)]



    # encode label for classification
    le = preprocessing.LabelEncoder()
    le.fit(df_1_5_star.stars)
    df_1_5_star['label'] = le.transform(df_1_5_star.stars)

    # train, test split
    X_train, X_test, y_train, y_test = train_test_split(df_1_5_star['text'], df_1_5_star['label'],
    ...
    w2v = gensim.models.Word2Vec(list(df_1_5_star.text), size=200, window=10, min_count=2, iter=10)
